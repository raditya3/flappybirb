...
class NeuroEvolution {
    ...
    constructor(json : {
        ...
        for(var i=0; i<this.populationSize;i++){
            ...
            if(this.populatePreExisting && !!localStorage.getItem("weights")){
                this.population[i].setWeights(localStorage.getItem("weights"),+localStorage.getItem("bias"));
            }
        }
    }
    ...
    evolve = () => {
        ...
        this.population =this.population.slice(0,this.populationSize*this.survivalFraction);
        ...
        for(let i = 0; i<repopulateNum*this.newSpecimenRate;i++){
            ...
            if(Math.random()<this.mutationChance)   specimen.mutate(this.mutagen,this.mutationSeverity);
