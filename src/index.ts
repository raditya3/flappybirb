...
const iterationIndicator =  <HTMLSpanElement> document.getElementById("Iteration")
...
async function GameBoard() {
  ...
  let survivors = true;
  let fitness = 0
const obstacleController = new ObstacleController(OBSTACLE_INBETWEEN_DISTANCE,  OBSTACLE_GAP, BIRD_VELOCITY, canvas);
  ...
  while(survivors){
    ...
    const totalAlive = birbPopulation.map(brb => {
