...
class Obstacles {
    ...
    constructor(meanDistance : number, gap : number, velocity : number, canvas : HTMLCanvasElement){
        this.gap = gap;
        this.canvas = canvas;
    ...
    update = () => {
        if(this.obstacleArray.length===0 || (this.canvas.width - last(this.obstacleArray).pos.x)>this.meanDistance){
